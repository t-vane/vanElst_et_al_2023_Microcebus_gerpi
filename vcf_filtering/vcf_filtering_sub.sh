...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/00_filter_indels_invariants.$set_id.oe $scripts_dir/00_filter_indels_invariants.sh $reference $vcf_raw $vcf_dir/allScaffolds.snps.vcf

## Filter for minimum depth
min_dp=5
mean_dp=5
sbatch --account=nib00015 --output=$vcf_dir/logFiles/01_filter_min-dp.$set_id.oe $scripts_dir/01_filter_min-dp.sh $vcf_dir/allScaffolds.snps.vcf $min_dp $mean_dp $vcf_dir/$set_id.allScaffolds.snps.01filt.vcf
...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/02_filter_missing-1.$set_id.oe $scripts_dir/02_filter_missing-1.sh \
...
suffix=auto
sbatch --account=nib00015 --output=$vcf_dir/logFiles/03_annot_gatk.$set_id.oe $scripts_dir/03_annot_gatk.sh \
...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/04_filter_gatk.$set_id.oe $scripts_dir/04_filter_gatk.sh \
...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/05_filter_max-dp.$set_id.oe $scripts_dir/05_filter_max-dp.sh \
...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/06_filter_missing-2.$set_id.oe $scripts_dir/06_filter_missing-2.sh \
...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/07_filter_outgroups.$set_id.oe $scripts_dir/07_filter_outgroups.sh \
...
## Annotate with INFO fields FisherStrand, RMSMappingQuality, MappingQualityRankSumTest, ReadPosRankSumTest and AlleleBalance
suffix=auto
sbatch --account=nib00015 --output=$vcf_dir/logFiles/03_annot_gatk.part.$set_id.oe $scripts_dir/03_annot_gatk.sh \
...
sbatch --account=nib00015 --output=$vcf_dir/logFiles/04_filter_gatk.part.$set_id.oe $scripts_dir/04_filter_gatk.sh \
...
## Filter for maximum depth as (mean depth + 2 * standard deviation) / number of individuals
sbatch --account=nib00015 --output=$vcf_dir/logFiles/05_filter_max-dp.part.$set_id.oe $scripts_dir/05_filter_max-dp.sh \
